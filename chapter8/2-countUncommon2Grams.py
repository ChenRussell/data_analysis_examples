...
def getNgrams(input, n):
    ...
    for i in range(len(input)-n+1):
        if(isCommon(input[i:i+n])==False):      # 我添加的
            ngramTemp = " ".join(input[i:i+n])
            if ngramTemp not in output:         # 存在指定键返回true
                output[ngramTemp] = 0
                # print(getFirstSentenceContaining(ngramTemp, content))
            output[ngramTemp] += 1
    return output
...
#     print(getFirstSentenceContaining(ngram,content))
